	...
	public void getCount() {
		...
		while (max < 3) {//第一層迴圈
			...
			if (count.getSign().equals("*") || count.getSign().equals("+") || count.getSign().equals("-")
				...
				if(count.getNumber1() < 2 || count.getNumber1() > 20
					|| count.getNumber2() < 2 || count.getNumber2() > 20) {//輸入完兩個數字後,判斷是否被限制
			...
			while ((count.getNumber1() >= 2 && count.getNumber1() <= 20)
				...
				if (count.getSign().equals("*")) {//取得原先的運算符號,看是哪一種便會掉進來
					a = (count.getNumber1() + count.getSign() + count.getNumber2() + "="
							+ (count.getNumber1() * count.getNumber2()));
//					to = count.getNumber1() * count.getNumber2();
					System.out.println(a + " 請問這是你輸入的算式嗎?");
					chick = scan.next();
